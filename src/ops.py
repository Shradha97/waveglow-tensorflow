...
var_initializer = tf.compat.v1.random_normal_initializer(stddev=0.02)
...
def get_var(name,
            ...
            weight_norm=False):
    if weight_norm:
        assert len(shape) == 4 #should be convolution
        g = tf.compat.v1.get_variable(name=name+magic+'_g', shape=[1, 1, 1, shape[-1]], dtype=dtype, initializer=initializer, trainable=trainable)
        v = tf.compat.v1.get_variable(name=name+magic+'_v', shape=shape, dtype=dtype, initializer=initializer, trainable=trainable)
        v_norm = tf.nn.l2_normalize(v, axis=[0, 1, 2])
        return g * v_norm
    ret = tf.compat.v1.get_variable(name=name, shape=shape, dtype=dtype, initializer=initializer, trainable=trainable)
    return ret
...
def mydeconv2d(inputs,
    ...
    if len(strides) == 2:
        strides = [1, 1] + strides
    with tf.compat.v1.variable_scope(scope or 'deconv', reuse=reuse):
        input_shape = inputs.get_shape().as_list()
        input_shape[2] = tf.shape(input=inputs)[2]
        ...
        biases = get_var("biases", shape=[output_channel], initializer=tf.compat.v1.zeros_initializer())
...
def myconv2d(inputs,
    ...
    if len(strides) == 2:
        strides = [1, 1] + strides
    with tf.compat.v1.variable_scope(scope or 'conv2d', reuse=reuse):
        ...
        biases = get_var("biases", shape=[output_channel], initializer=tf.compat.v1.zeros_initializer())
        conv = tf.nn.conv2d(input=inputs, filters=weights, strides=strides, padding=padding, data_format="NCHW", dilations=dilations)
...
def inv1x1conv2d(inputs, reverse=False, scope=None, reuse=None):
    with tf.compat.v1.variable_scope(scope or 'inv1x1_2d', reuse=reuse):
        ...
        if not reverse:
            ...
            logdet = tf.cast(tf.math.log(absdet + 1e-12), tf.float32) - tf.math.log(1e+3) * (inputs.get_shape().as_list()[1])
            logdet *= tf.cast(tf.shape(input=inputs)[0] * tf.shape(input=inputs)[2] * tf.shape(input=inputs)[3], tf.float32)
            ret = tf.nn.conv2d(input=inputs, filters=w, strides=[1, 1, 1, 1], padding='SAME', data_format="NCHW")
            return ret, logdet
        w = tf.linalg.inv(w)
        ret = tf.nn.conv2d(input=inputs, filters=w, strides=[1, 1, 1, 1], padding='SAME', data_format="NCHW")
        return ret

...
class WaveGlow():
    ...
    def build_model(self):
        self.global_step = tf.compat.v1.get_variable('global_step', initializer=0, dtype=tf.int32, trainable=False)
        self.mels = tf.compat.v1.placeholder(tf.float32, [None, args.n_mel, None])
        self.wavs = tf.compat.v1.placeholder(tf.float32, [None, args.squeeze_size, args.wav_time_step // args.squeeze_size])
        ...
        self.conditions = tf.transpose(a=self.conditions, perm=[0, 2, 1])
        self.conditions = tf.reshape(self.conditions, [-1, tf.shape(input=self.conditions)[1] // args.squeeze_size, args.squeeze_size * args.n_mel])
        self.conditions = tf.transpose(a=self.conditions, perm=[0, 2, 1])
        self.z = []
        ...
        total_size = tf.cast(tf.size(input=self.z), tf.float32)
        self.logdet_loss = -tf.reduce_sum(input_tensor=self.logdets) / total_size
        self.logs_loss = -tf.reduce_sum(input_tensor=self.logss) / total_size
        self.prior_loss = tf.reduce_sum(input_tensor=self.z ** 2 / (2 * args.sigma ** 2)) / total_size
...
########INFERENCE#########
        self.output = tf.random.truncated_normal([tf.shape(input=self.conditions)[0], args.output_remain, tf.shape(input=self.conditions)[2]], dtype=tf.float32)
        for i in reversed(range(args.n_flows)):
            if (i + 1) % args.early_output_every == 0 and (i + 1) != args.n_flows:
                self.newz = tf.random.truncated_normal([tf.shape(input=self.conditions)[0], args.early_output_size, tf.shape(input=self.conditions)[2]], stddev=args.infer_sigma)
                self.output = tf.concat([self.newz, self.output], axis=1)
            self.output = conv_afclayer(self.output, self.conditions, reverse=True, scope='afc_'+str(i+1), reuse=tf.compat.v1.AUTO_REUSE)
########INFERENCE#########

    def train(self):
        self.data_generator = multiproc_reader(500)
        self.validation_data_generator = multiproc_reader_val(args.sample_num * 5)
        self.lr = tf.compat.v1.train.exponential_decay(args.lr,
                                             self.global_step,
                                             args.lr_decay_steps,
                                             args.lr_decay_rate)
        ...
        varset = list(set(tf.compat.v1.global_variables()) | set(tf.compat.v1.local_variables()))
        self.saver = tf.compat.v1.train.Saver(var_list=varset, max_to_keep=3)
        ...
        self.writer = tf.compat.v1.summary.FileWriter(args.summary_dir)
        with tf.compat.v1.name_scope("summaries"):
            self.s_logdet_loss = tf.compat.v1.summary.scalar('logdet_loss', self.logdet_loss)
            self.s_logs_loss = tf.compat.v1.summary.scalar('logs_loss', self.logs_loss)
            self.s_prior_loss = tf.compat.v1.summary.scalar('prior_loss', self.prior_loss)
            self.s_loss = tf.compat.v1.summary.scalar('total_loss', self.loss)
            self.merged = tf.compat.v1.summary.merge([self.s_logdet_loss, self.s_logs_loss, self.s_prior_loss, self.s_loss])
